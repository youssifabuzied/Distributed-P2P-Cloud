...
impl eframe::App for GuiApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        while let Ok(evt) = self.evt_rx.try_recv() {
            match evt {
                UiEvent::Queued(id) => {
                    // For now, just print to console; main UI will reflect tracker
                ...
                for (id, status) in self.client.tracker.list_all() {
                    ui.horizontal(|ui| match status {
                        ...
                        RequestStatus::Completed(ref resp) => {
                            ui.label(format!("#{} | ✓ {}", id, resp.output_path.as_ref().unwrap_or(&"N/A".to_string())));
                        }
...
fn main() {
    // Configuration like client_app.rs uses
    let username = "user_gui";
    let client_ip = "127.0.0.1";
    let client_port = 8081u16;
    let middleware_addr = "127.0.0.1:9000";

    let client = Arc::new(Client::new(username, client_ip, client_port, middleware_addr));
    ...
    eframe::run_native("Distributed P2P Client", native_options, Box::new(move |_cc| Box::new(app)))
